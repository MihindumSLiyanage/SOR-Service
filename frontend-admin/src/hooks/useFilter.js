...
const useFilter = (data) => {
  ...
  const serviceData = useMemo(() => {
    ...
    if (location.pathname === '/dashboard') {
      const reservationPending = services.filter(
        (statusP) => statusP.status === 'Pending'
      );
      setPending(reservationPending);

      const reservationProcessing = services.filter(
        (statusO) => statusO.status === 'Processing'
      );
      setProcessing(reservationProcessing);

      const reservationCompleted = services.filter(
        (statusD) => statusD.status === 'Completed'
      );
      setCompleted(reservationCompleted);
    ...
    if (searchReservation) {
      services = services.filter(
        (search) =>
          search?.vehicleType?.toLowerCase().includes(searchReservation.toLowerCase()) ||
          search?.serviceType?.toLowerCase().includes(searchReservation.toLowerCase()) ||
          search?.userName?.toLowerCase().includes(searchReservation.toLowerCase()) ||
          search?.vehicleNo?.toLowerCase().includes(searchReservation.toLowerCase())
      );
    }

    if (slot) {
      services = services.filter((reservation) => reservation.slot === slot);
    }
    ...
    if (active) {
      services = services.filter((products) => products.active === active);
    }
    ...
    if (categoryType) {
      services = services.filter((search) =>
        search.parent.toLowerCase().includes(categoryType.toLowerCase())
      );
    }
  ...
  const handleSubmitReservation = (e) => {
    e.preventDefault();
    setSearchReservation(reservationRef.current.value);
  };
